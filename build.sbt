...
libraryDependencies ++= Seq(
  ...
  "dvla" %% "vehicles-presentation-common" % "2.31" withSources() withJavadoc() exclude("junit", "junit-dep"),
  "dvla" %% "vehicles-presentation-common" % "2.31" % "test" classifier "tests"  withSources() withJavadoc() exclude("junit", "junit-dep"),
...
// ====================== Sandbox Settings ==========================
lazy val osAddressLookupProject = osAddressLookup("0.19").disablePlugins(PlayScala, SbtWeb)
lazy val vehicleAndKeeperLookupProject = vehicleAndKeeperLookup("0.16").disablePlugins(PlayScala, SbtWeb)
lazy val vehiclesDisposeFulfilProject = vehiclesDisposeFulfil("0.11").disablePlugins(PlayScala, SbtWeb)
lazy val emailServiceProject = emailService("0.12").disablePlugins(PlayScala, SbtWeb)
