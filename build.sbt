...
libraryDependencies ++= Seq(
  ...
  "dvla" %% "vehicles-presentation-common" % "2.34" withSources() withJavadoc() exclude("junit", "junit-dep"),
  "dvla" %% "vehicles-presentation-common" % "2.34" % "test" classifier "tests"  withSources() withJavadoc() exclude("junit", "junit-dep"),
...
// ====================== Sandbox Settings ==========================
lazy val osAddressLookupProject = osAddressLookup("0.22").disablePlugins(PlayScala, SbtWeb)
lazy val vehicleAndKeeperLookupProject = vehicleAndKeeperLookup("0.18").disablePlugins(PlayScala, SbtWeb)
lazy val vehiclesDisposeFulfilProject = vehiclesDisposeFulfil("0.13").disablePlugins(PlayScala, SbtWeb)
lazy val emailServiceProject = emailService("0.15").disablePlugins(PlayScala, SbtWeb)
