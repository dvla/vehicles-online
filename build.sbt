...
libraryDependencies ++= Seq(
  ...
  // VMPR
  "dvla" %% "vehicles-presentation-common" % "2.60-SNAPSHOT" withSources() withJavadoc() exclude("junit", "junit-dep"),
  "dvla" %% "vehicles-presentation-common" % "2.60-SNAPSHOT" % "test" classifier "tests" withSources() withJavadoc() exclude("junit", "junit-dep")
)
...
lazy val osAddressLookupProject = osAddressLookup("0.33-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val vehicleAndKeeperLookupProject = vehicleAndKeeperLookup("0.27-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val vehiclesDisposeFulfilProject = vehiclesDisposeFulfil("0.21-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)

SandboxSettings.portOffset := 17000

SandboxSettings.applicationContext := "sell-to-the-trade"

SandboxSettings.webAppSecrets := "vehicles-online/conf/vehiclesOnline.conf"
