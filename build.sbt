...
libraryDependencies ++= Seq(
  ...
  "dvla" %% "vehicles-presentation-common" % "2.35-SNAPSHOT" withSources() withJavadoc() exclude("junit", "junit-dep"),
  "dvla" %% "vehicles-presentation-common" % "2.35-SNAPSHOT" % "test" classifier "tests"  withSources() withJavadoc() exclude("junit", "junit-dep"),
...
// ====================== Sandbox Settings ==========================
lazy val osAddressLookupProject = osAddressLookup("0.23-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val vehicleAndKeeperLookupProject = vehicleAndKeeperLookup("0.19-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val vehiclesDisposeFulfilProject = vehiclesDisposeFulfil("0.14-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
lazy val emailServiceProject = emailService("0.16-SNAPSHOT").disablePlugins(PlayScala, SbtWeb)
