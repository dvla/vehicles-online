...
class DisposeIntegrationSpec extends UiSpec with TestHarness {
  "go to page" should {
    "display the page" taggedAs UiTag in new WebBrowser {
      ...
      page.url should equal(DisposeForPrivateKeeperPage.url)
    ...
    "display optional email for private keeper" taggedAs UiTag in new WebBrowser {
      go to BeforeYouStartPage
      cacheSetup()
  ...
  "dispose button" should {
    "display DisposeSuccess on correct submission with no email confirmation" taggedAs UiTag in new WebBrowser {
      go to BeforeYouStartPage
    ...
    "display validation errors when all mandatory fields entered but" +
      " no email preference selected" taggedAs UiTag in new WebBrowser {
      go to BeforeYouStartPage
      cacheSetup()
      go to DisposeForPrivateKeeperPage
