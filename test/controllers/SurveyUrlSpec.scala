...
class SurveyUrlSpec extends UnitSpec {
  ...
  "when trader" should {
    "get the business survey url if configured so and for the first time" in new WithApplication {
      ...
      surveyUrl(request, false) should equal(Some(surveyUrlString))
    }
    ...
    "Not get the survey url if within the timeout " in new WithApplication {
      ...
      surveyUrl(request, false) should equal(None)
    }
  ...
  "when private keeper" should {
    "get the business survey url if configured so and for the first time" in new WithApplication {
      ...
      surveyUrl(request, true) should equal(Some(privateKeeperSurveyUrlString))
    }
    ...
    "Not get the survey url if within the timeout " in new WithApplication {
      ...
      surveyUrl(request, true) should equal(None)
    }
